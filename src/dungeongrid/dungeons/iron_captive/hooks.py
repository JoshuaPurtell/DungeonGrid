...
from dungeongrid.dungeons.hook_recipes import achievement_effect, message_after, objective_flag
...
def register(registry):
    ...
    registry.on("after_MessageEffect", message_after("captive_freed", "messaged_after_rescue", achievement={"id": "coordinate_the_rescue_route", "title": "Coordinate the Rescue Route", "reward": 0.12, "description": "Send a message after the captive is freed."}))
...
def _on_warden_cleanup(ctx):
    ...
    if ticks % 2 == 1:
        effects.extend([ModifyAlert(amount=1), EmitEvent(message="The freed captive coughs against the iron dust. Alert rises by 1.")])
    return effects
