...
def on_load(ctx):
    ...
    ctx.emit("Water carries a distant bell note. The rite will escalate if the bell keeper is ignored.")
...
def _after_damage(ctx):
    ...
    if target and target.role == "gloom_cultist" and not target.alive:
        return [SetFlag(key="bell_keeper_silenced", value=True), EmitEvent(message="The bell note falters as the keeper falls.")]
    return []


def _on_warden_cleanup(ctx):
    state = ctx.state
    if state.done or state.scripts.get("bell_alarm_triggered") or state.scripts.get("bell_keeper_silenced"):
        return []
    ...
    if timer > 0:
        effects.append(EmitEvent(message=f"The bell keeper chants. Alarm will ring in {timer} Warden phase."))
        return effects
    effects.extend(
        [
            SetFlag(key="bell_alarm_triggered", value=True),
            ModifyAlert(amount=3),
            achievement_effect(id="survive_bell_alarm", title="Survive the Bell Alarm", reward=0.16, description="Endure the first Warden alarm pulse."),
            EmitEvent(message="The bell rings under blackwater. Reinforcements surface from the channel."),
        ]
    )
    for spawn in (
