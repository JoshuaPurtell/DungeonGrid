...
def _after_disarm(ctx):
    if ctx.state.scripts.get("ember_oath_taken"):
        return [
            ...
            achievement_effect(id="keep_calm_after_the_oath", title="Keep Calm After the Oath", reward=0.12, description="Disarm a hazard after the objective wakes the trial."),
        ]
    return []
...
def _on_warden_cleanup(ctx):
    ...
    if pulses == 1:
        ...
        effects.append(EmitEvent(message="The lower embers knit themselves into a skittering watcher."))
        if spawn:
            effects.append(spawn)
    elif pulses % 2 == 0:
        effects.extend([ModifyAlert(amount=1), EmitEvent(message="The oath answers with a hot pulse. Alert rises by 1.")])
    return effects
