...
def _on_objective_taken(ctx):
    return [SetFlag(key="cinder_relic_claimed", value=True), SetFlag(key="carrier_pressure", value=0)]


def _on_warden_cleanup(ctx):
    state = ctx.state
    carrier = state.heroes.get(state.objective.carrier or "")
    if state.done or not state.scripts.get("cinder_relic_claimed") or not carrier or not carrier.alive:
        return []
    ...
    if "carrier_wrap" in carrier.inventory or "oath_shield" in carrier.inventory:
        effects.append(achievement_effect(id="guarded_relic_carrier", title="Guarded Relic Carrier", reward=0.14, description="Give the relic carrier protective support."))
    if next_pressure >= 2:
        effects.append(achievement_effect(id="hold_relic_under_pressure", title="Hold the Relic Under Pressure", reward=0.16, description="Keep the relic carrier alive through two Warden pulses."))
    return effects
