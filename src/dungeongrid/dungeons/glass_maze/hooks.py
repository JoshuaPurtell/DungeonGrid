...
def register(registry):
    ...
    registry.on("on_furniture_searched", clue_flags({"mirror_stand_1", "lower_map_1"}, "maze_clue_read"))
...
def _after_message(ctx):
    ...
    if ctx.state.scripts.get("mirror_prism_taken"):
        effects.extend(message_after("mirror_prism_taken", "shared_prism_route", achievement={"id": "call_the_prism_route", "title": "Call the Prism Route", "reward": 0.10, "description": "Send a route message after taking the prism."})(ctx))
    if ctx.state.scripts.get("maze_clue_read"):
        effects.append(achievement_effect(id="share_the_maze_clue", title="Share the Maze Clue", reward=0.12, description="Read a maze clue and send a party message."))
    return effects
...
def _on_warden_cleanup(ctx):
    ...
    if pulses == 1:
        ...
        effects.extend([ModifyAlert(amount=1), EmitEvent(message="The prism flashes through the walls. Alert rises by 1.")])
    return effects
