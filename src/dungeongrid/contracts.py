...
RULES: dict[str, str] = {
    "turn": "Heroes act one at a time with 3 AP. Submit a queued JSON action plan for only the active hero. Execution stops at AP/turn end, episode end, or a tactical reveal boundary. Safe partial map reveals may allow later queued actions to continue.",
    "actions": "Actions are JSON objects with a type field. Common types: move {direction}; sneak {direction}; open_door {target}; attack_melee {target}; attack_ranged {target}; attack_object {target}; cast {target,payload:{spell}}; inspect_tile {target:[x,y]}; search_traps {}; search_secrets {}; search_treasure {target}; search_furniture {target}; disarm {target}; interact {target}; distract {target}; sabotage {target}; rig_trap {target}; use_item {target}; equip_item {target}; give_item {target,payload:{item}}; message {target,payload:{text}}; guard {}; call_extraction {}; end_turn {}. Coordinates are only valid for inspect_tile. Object/entity actions target ids from visible_objects or visible_entities. Spell targets depend on the spell.",
    "goblin_heists": "Goblin mode favors heist play over clearing every defender. The alert meter is shown as quiet, suspicious, alarmed, or lockdown. sneak moves quietly and can dampen alarm near shadows or cover. distract lures or stalls visible defenders and decoy objects. sabotage disables adjacent bells, runes, grates, locks, gears, or traps. rig_trap turns adjacent objects, doors, or traps into snares that delay defenders nonlethally. Rewards and achievements favor stealing the objective, misdirection, optional loot, nonlethal handling, and fast extraction.",
    "movement": "move costs 1 AP and uses direction north/south/east/west. sneak also costs 1 AP in goblin mode and uses the same cardinal directions while tracking stealth progress. Only move into adjacent unblocked floor tiles. Closed doors, walls, occupied tiles, and undiscovered secret doors block movement. If a direction was just reported blocked from your current tile, choose a different action.",
    ...
    "doors_rooms": "open_door costs 1 AP on adjacent known closed doors. Opening a door reveals new room state. Queued plans continue through safe empty-room reveals, but stop when the reveal changes tactical facts such as visible enemies, revealed traps, objective visibility, secret discovery, boss state, objective state, or turn/episode state. Some monsters wake immediately when a room is revealed; others stay dormant until line of sight, alarm/alert, or objective pressure wakes them.",
