# Copied from legacy dungeons/black_standard_keep/hooks.py during tier migration.
...
def register(registry):
    ...
    registry.on(
        ...
        clue_flags(
            {"signal_table_1"}, "watch_rotation_read", "watch_rotation_read_before_standard"
        ),
    )
    ...
    registry.on(
        "after_MessageEffect",
        message_after(
            "black_standard_taken",
            "standard_retreat_called",
            achievement={
                "id": "call_the_standard_retreat",
                "title": "Call the Standard Retreat",
                "reward": 0.10,
                "description": "Send a message after the black standard is taken.",
            },
        ),
    )
    registry.on(
        ...
        guard_after(
            ...
            "standard_line_held",
            achievement={
                "id": "hold_the_keep_line",
                "title": "Hold the Keep Line",
                "reward": 0.12,
                "description": "Use guard after the black standard is taken.",
            },
        ),
    )
    registry.on(
        ...
        attack_after(
            "black_standard_taken",
            "standard_retreat_covered",
            ranged=True,
            achievement={
                "id": "cover_the_keep_retreat",
                "title": "Cover the Keep Retreat",
                "reward": 0.12,
                "description": "Make a ranged attack after the black standard is taken.",
            },
        ),
    )
...
def _intel_achievement(ctx):
    if ctx.state.scripts.get("watch_rotation_read_before_standard"):
        return [
            achievement_effect(
                id="breach_with_intel",
                title="Breach With Intel",
                reward=0.12,
                description="Read the keep watch rotation before carrying the standard.",
            )
        ]
    return []
...
def _on_warden_cleanup(ctx):
    ...
    effects = []
    if (
        any(m.alive and m.role == "gloom_cultist" for m in state.monsters.values())
        and state.round % 3 == 0
    ):
        effects.extend(
            [
                ModifyAlert(amount=1),
                EmitEvent(message="A keep caller beats the signal rhythm. Alert rises by 1."),
            ]
        )
    ...
    if ticks == 1:
        ...
        effects.extend(
            [
                ModifyAlert(amount=1),
                EmitEvent(
                    message="The black standard drags attention like a torch. Alert rises by 1."
                ),
            ]
        )
    return effects
