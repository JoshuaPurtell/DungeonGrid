# Copied from legacy dungeons/magistrate_gold/hooks.py during tier migration.
...
def register(registry):
    ...
    registry.on(
        ...
        clue_flags({"ledger_table_1"}, "ledger_checked", "ledger_checked_before_strongbox"),
    )
    ...
    registry.on(
        ...
        message_after(
            "strongbox_taken",
            "planned_strongbox_retreat",
            achievement={
                "id": "call_the_getaway",
                "title": "Call the Getaway",
                "reward": 0.10,
                "description": "Send a message after taking the strongbox.",
            },
        ),
    )
...
def _ledger_achievement(ctx):
    if ctx.state.scripts.get("ledger_checked_before_strongbox"):
        return [
            achievement_effect(
                id="audit_before_theft",
                title="Audit Before Theft",
                reward=0.12,
                description="Check the ledger before carrying the strongbox.",
            )
        ]
    return []
...
def _on_warden_cleanup(ctx):
    ...
    if not state.scripts.get("seal_guard_spawned"):
        ...
        effects = [
            SetFlag(key="seal_guard_spawned", value=True),
            EmitEvent(message="The seal chime pulls a guard out from the lower vault."),
        ]
        if spawn:
    ...
    return [
        ...
        EmitEvent(message="The strongbox keeps chiming. Alert rises by 1."),
    ]
