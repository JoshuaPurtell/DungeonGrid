# Copied from legacy dungeons/barrow_of_the_hollow_king/hooks.py during tier migration.
...
def register(registry):
    ...
    registry.on(
        ...
        clue_flags(
            {"ancestor_masks_1", "crypt_marker_1"},
            "funeral_path_read",
            "funeral_path_read_before_crown",
        ),
    )
    registry.on(
        "after_MessageEffect",
        message_after(
            "hollow_crown_taken",
            "crown_retreat_called",
            achievement={
                "id": "call_the_crown_retreat",
                "title": "Call the Crown Retreat",
                "reward": 0.10,
                "description": "Send a message after the hollow crown is taken.",
            },
        ),
    )
    registry.on(
        ...
        guard_after(
            ...
            "crown_line_held",
            achievement={
                "id": "hold_back_the_barrow",
                "title": "Hold Back the Barrow",
                "reward": 0.12,
                "description": "Use guard after the hollow crown is taken.",
            },
        ),
    )
    registry.on(
        ...
        attack_after(
            "hollow_crown_taken",
            "stood_against_undead",
            ranged=False,
            achievement={
                "id": "stand_against_the_dead",
                "title": "Stand Against the Dead",
                "reward": 0.12,
                "description": "Make a melee attack after the hollow crown is taken.",
            },
        ),
    )
...
def _route_achievement(ctx):
    if ctx.state.scripts.get("funeral_path_read_before_crown"):
        return [
            achievement_effect(
                id="know_the_funeral_path",
                title="Know the Funeral Path",
                reward=0.12,
                description="Read the funeral path before carrying the crown.",
            )
        ]
    return []
...
def _on_warden_cleanup(ctx):
    ...
    if ticks == 1:
        ...
        effects.append(
            EmitEvent(message="The Hollow King's shadow separates from the throne wall.")
        )
        if spawn:
        ...
        effects.extend(
            [
                ...
                EmitEvent(
                    message="The crown whispers the carrier's route aloud. Alert rises by 1."
                ),
            ]
        )
    return effects
