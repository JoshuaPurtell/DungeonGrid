...
def _draw_knocked_out(
    ...
    draw.rectangle((x1 + 3 * scale, y1 + 9 * scale, x2 - 3 * scale, y1 + 13 * scale), fill=(36, 32, 29), outline=OUTLINE)
...
def _draw_hud(
    ...
    for line in lines:
        draw.text((x + 14, yy), line, fill=GOLD if line == str((state.get("game_mode") or {}).get("party_label") or "Party").title() else MUTED, font=font)
        yy += 20
    for hero in (state.get("heroes") or {}).values():
